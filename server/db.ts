...
import { InsertUser, users, products, orders, Product, InsertProduct, Order, InsertOrder } from "../drizzle/schema";
...
export async function updateOrder(id: number, updates: Partial<InsertOrder>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.update(orders).set(updates).where(eq(orders.id, id));
}
